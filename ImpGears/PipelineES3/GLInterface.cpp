...
#include <PipelineES3/GlslCode.h>
...
void GlPlugin::init()
{
    if(s_internalState == nullptr)
    {
        ...
        s_internalState->_needInverseMat = false;// true; // (major<3) || (major==3 && minor==0);
    }
}
...
int GlPlugin::load(ReflexionModel::Ptr& program)
{
    ...
    static const std::string glsl_version = "#version 300 es\n"; // OpenGL ES 3.0
    // static const std::string glsl_float_precision = "precision mediump float;\n";
    static const std::string glsl_float_precision = "precision highp float;\n";

    if( program->getRenderData() ) return 0;

    std::string fullVertCode = glsl_version + glsl_float_precision;
    std::string fullFragCode = glsl_version + glsl_float_precision;
