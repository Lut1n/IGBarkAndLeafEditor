...
void main()
{
    ...
    fragmentOut[0] = out_color;
    fragmentOut[1] = out_emissive;
    fragmentOut[2] = vec4(out_normal,1.0);
    fragmentOut[3] = vec4(vec3(out_reflectivity),1.0);
    fragmentOut[4] = vec4(vec3(out_shininess),1.0);
    fragmentOut[5] = vec4(vec3(out_depth),1.0);
}
